...
from models.drift_loss import NFDriftLossFunction
...
if __name__ == '__main__':
    ...
    parser.add_argument('--drift_loss_weight', default=0.5, type=float, help='drift_loss ratio to MLE loss')
    parser.add_argument('--lr', default=1e-4, type=float, help='Maximum learning rate')
    parser.add_argument('--lr_schedule_type', default='wsd', type=str, choices=["wsd", "ws", "d", "cos"], help='Learning rate schedule')
    ...
    nf_drift_loss_model = NFDriftLossFunction()
    ...
    for epoch in tqdm(range(start_epoch, start_epoch + args.epochs)):
        ...
        # Stop grad
        pos_samples_mle = pos_samples.clone().to(device)
        pos_samples_mle = pos_samples_mle.reshape(args.batch_size * pos_per_sample, num_patches, num_classes)
        ...
        z_drift, _, log_dets_drift = model(x_gen_detached, y=None)
        nf_loss = 0.5 * z_drift.pow(2).mean(dim=(-1, -2)) - log_dets_drift      # (B*N_gen, )
        ...
        drift_loss = nf_drift_loss_model(x_gen, pos_samples, nf_score)
        drift_loss_mean = drift_loss.mean()
        
        # TarFlow MLE Loss
        eps = torch.randn_like(pos_samples_mle)
        pos_samples_mle = pos_samples_mle + eps
        z_mle, _, log_dets_mle = model(pos_samples_mle, y=None)
        mle_loss = 0.5 * z_mle.pow(2).mean(dim=(-1, -2)) - log_dets_mle
        mle_loss_mean = mle_loss.mean()
                        
        # print(f"drift:{drift_loss.shape} / mle:{mle_loss.shape}")
        curr_loss = mle_loss_mean + args.drift_loss_weight * drift_loss_mean
        ...
        if (epoch + 1) % args.sample_freq == 0:
            ...
            with torch.no_grad():
                with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                    generated_samples_eval = model.reverse(noise_eval, y=None)
